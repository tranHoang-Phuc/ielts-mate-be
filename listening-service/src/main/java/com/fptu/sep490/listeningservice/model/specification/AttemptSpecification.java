...
public class AttemptSpecification {
    public static Specification<Attempt> byConditions(
        ...
        return ((root, query, criteriaBuilder) -> {
            Join<Attempt, ListeningTask> task = root.join("listeningTask", JoinType.INNER);
            List<Predicate> predicates = new ArrayList<>();
            if(listeningTaskId != null) {
                predicates.add(criteriaBuilder.equal(task.get("taskId"), listeningTaskId));
            }
