...
public class VerifyEmailRateLimiterImplTest {

	private RedisService redisService;
	private VerifyEmailRateLimiterImpl limiter;

	@BeforeEach
	void setUp() {
		redisService = Mockito.mock(RedisService.class);
		limiter = new VerifyEmailRateLimiterImpl(redisService);
	...
	void isBlocked_noCount_resetsAndReturnsFalse() throws Exception {
		...
		when(redisService.getValue(eq("verify-email:attempts:" + email), eq(Integer.class))).thenReturn(null);

		boolean blocked = limiter.isBlocked(email);
		assertFalse(blocked);
		verify(redisService).delete(eq("verify-email:attempts:" + email));
	}

	@Test
	void isBlocked_belowThreshold_returnsFalse() throws Exception {
		String email = "user@example.com";
		when(redisService.getValue(eq("verify-email:attempts:" + email), eq(Integer.class))).thenReturn(2);
	...
	void isBlocked_atThreshold_returnsTrue() throws Exception {
		String email = "user@example.com";
		when(redisService.getValue(eq("verify-email:attempts:" + email), eq(Integer.class))).thenReturn(3);
	...
	void recordAttempt_firstTime_setsToOneWithTtl() throws Exception {
		String email = "user@example.com";
		String key = "verify-email:attempts:" + email;
